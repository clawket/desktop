/**
 * LM-87 — Decomposition Suggestion Panel (desktop port of
 * `clawket/web/src/features/decomposition/SuggestionPanel.tsx`).
 *
 * Workflow:
 *   1. Fetch `POST /tasks/:id/decompose` on mount. Each row of the daemon's
 *      `success_criteria`-derived list becomes one suggested subtask.
 *   2. User selects a subset (checkbox per row).
 *   3. User reorders selected suggestions (drag handle + up/down keyboard
 *      fallback). Reorder dictates the `idx` sent to the daemon so the tree
 *      mirrors the panel order.
 *   4. "Accept N" calls `POST /tasks/:id/subtasks` once per selected
 *      suggestion. After every call resolves, `onAccepted()` fires so the
 *      parent (TaskDetailPanel) can refresh the subtask list.
 *
 * Drag-drop uses native HTML5 dragstart/dragover/drop (parity with web).
 * jsdom does not fully simulate drag events, so unit tests exercise the
 * up/down keyboard fallback for ordering coverage.
 */
import { useEffect, useMemo, useState } from "react";
import { Button } from "@clawket/ui";
import type {
  DecompositionResult,
...
export interface SuggestionPanelProps {
  taskId: string;
  /** Daemon decomposition call. Resolves to the suggestion list. */
  onDecompose: (
    id: string,
    args?: DecomposeTaskArgs,
  ) => Promise<DecompositionResult>;
  /** Subtask creation — called once per accepted suggestion. */
  onCreateSubtask: (parentId: string, input: CreateSubtaskInput) => Promise<unknown>;
  /** Fired after every accepted suggestion has been persisted. Parent uses
   *  this to refetch its tree / subtask list. */
  onAccepted?: () => void;
}

interface RowState {
  /** Stable key for React + drag identity — the daemon's `idx` is unique
   *  within a single decompose response. */
  key: number;
...
export default function SuggestionPanel({
  ...
  useEffect(() => {
    let cancelled = false;
    onDecompose(taskId, { strategy: "auto", max_depth: 2 })
      .then((result) => {
        if (cancelled) return;
        setState({
          taskId,
  ...
  const isLoading =
    state.taskId !== taskId || (state.result === null && state.error === null);
  ...
  async function accept() {
    ...
    setAcceptError(null);
    try {
      // Sequential — the daemon assigns idx based on position relative to
      // existing children; running serially keeps the order stable and lets
      // us surface the first error without partial fan-out.
      for (let i = 0; i < selected.length; i++) {
        const s = selected[i];
        await onCreateSubtask(taskId, {
  ...
  if (isLoading) {
    return (
      <div
        ...
        className="text-label-sm text-muted py-2"
        aria-busy="true"
      >
        Loading suggestions...
      </div>
  ...
  if (state.taskId === taskId && state.error) {
    return (
      <div
        ...
        role="alert"
        className="text-label-sm text-danger py-2"
      >
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between text-label-sm text-muted">
        <span>
          ...
          {result.existing_children_count > 0 && (
            <>
              {" "}
              · {result.existing_children_count} existing child
              {result.existing_children_count === 1 ? "" : "ren"}
            </>
          )}
        </span>
      ...
      {result.policy_violations.length > 0 && (
        <ul
          data-testid="suggestion-panel-violations"
          className="space-y-1"
        ...
        >
          {result.policy_violations.map((v, i) => (
            <li
              key={`${v.field}-${i}`}
              role="alert"
              className={`text-label-sm ${v.severity === "error" ? "text-danger" : "text-warning"}`}
      ...
      {acceptError && (
        <div
          data-testid="suggestion-panel-accept-error"
          role="alert"
      ...
      <div className="flex justify-end">
        <Button
          ...
          onClick={accept}
          data-testid="suggestion-panel-accept"
        >
...
interface SuggestionRowProps {
  ...
  onMoveUp: () => void;
  onMoveDown: () => void;
  /** Native HTML5 drag-drop reorder hook: fires when another row is dropped
   *  onto this one. Caller resolves both keys to indexes. */
...
function SuggestionRow({
  ...
  return (
    <li
      ...
      data-testid={`suggestion-row-${row.key}`}
      draggable
      onDragStart={(e) => {
    ...
    >
      <span
        className="text-muted cursor-grab select-none pt-0.5"
        aria-hidden
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-label-sm text-muted mt-0.5">
      ...
      <div className="flex flex-col gap-0.5">
        <button
          ...
          disabled={isFirst}
          onClick={onMoveUp}
          className="text-label-sm text-muted hover:text-foreground disabled:opacity-30 disabled:cursor-not-allowed px-1"
        >
          ▲
        </button>
        <button
