...
describe("DetailBreadcrumb", () => {
  ...
  it("renders Plan > Unit > Task chain with the task as current", () => {
    ...
    expect(
      screen.getByTestId("detail-breadcrumb-plan").querySelector("button"),
    ).not.toBeNull();
    expect(
      screen.getByTestId("detail-breadcrumb-unit").querySelector("button"),
    ).not.toBeNull();
  ...
  it("fires onSelectItem with the clicked crumb identity", () => {
    ...
    fireEvent.click(
      screen.getByTestId("detail-breadcrumb-plan").querySelector(
        "button",
      ) as HTMLElement,
    );
    expect(onSelectItem).toHaveBeenCalledWith({ type: "plan", id: "PLAN-1" });
    fireEvent.click(
      screen.getByTestId("detail-breadcrumb-unit").querySelector(
        "button",
      ) as HTMLElement,
    );
  ...
  it("disables crumbs when onSelectItem is omitted", () => {
    ...
    const planBtn = screen
      .getByTestId("detail-breadcrumb-plan")
