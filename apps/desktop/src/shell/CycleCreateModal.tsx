...
import type { CreateCycleInput } from "../data/api";

export interface CycleCreateModalProps {
  /**
   * Project the cycle belongs to. Required by the daemon.
   */
  projectId: string;
  /**
   * Unit the cycle belongs to. Required by the daemon (PDD A4: Cycle ⊂ Unit);
   * the daemon rejects empty unit_id with MISSING_UNIT_ID.
   */
  unitId: string;
  onClose: () => void;
...
export function CycleCreateModal({
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      setErr((e as Error).message || "Failed to create cycle");
      setSubmitting(false);
    }
  }
